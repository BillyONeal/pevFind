...
class consoleParser 
{
    ...
    boost::shared_ptr<criterion> andParse();
    boost::shared_ptr<criterion> orParse();
    boost::shared_ptr<criterion> xorParse();
    boost::shared_ptr<criterion> exprParse();

    //Helpers for the R-D Parser
    bool isExpressionArgumentType();
    boost::shared_ptr<criterion> createBracket();
    boost::shared_ptr<criterion> createIf();
    void createModifier(std::vector<boost::shared_ptr<criterion> > &results);
    ...
    template <typename hash_t> boost::shared_ptr<criterion> createHashList(commandToken& token, std::size_t hashNameLen);
    template <typename hash_t> boost::shared_ptr<criterion> createHash(commandToken& token, std::size_t hashNameLen);
    void ascendingSorts(commandToken& token);
    void descendingSorts(commandToken& token);
    void parseTypeString(commandToken& token, std::vector<boost::shared_ptr<criterion> > &results);
    ...
    template <typename lowerClass, typename upperClass> 
        void subDateType(std::wstring& token, std::vector<boost::shared_ptr<criterion> > &results);
...
public:
    boost::shared_ptr<criterion> parseCmdLine(const std::wstring& commandLine);
};


template <typename hash_t> boost::shared_ptr<criterion> consoleParser::createHashList(commandToken& token, std::size_t hashNameLen)
{
    removeArgument(hashNameLen, token.argument);
    boost::shared_ptr<criterion> crit(new hash_t(loadStringsFromFile(getEndOrOption(token))));
    token.argument.clear();
    return crit;
}

template <typename hash_t> boost::shared_ptr<criterion> consoleParser::createHash(commandToken& token, std::size_t hashNameLen)
{
    removeArgument(hashNameLen, token.argument);
    boost::shared_ptr<criterion> result(new hash_t(getEndOrOption(token)));
    token.argument.clear();
    return result;
}

template <typename lowerClass, typename upperClass> 
void consoleParser::subDateType(std::wstring& token, std::vector<boost::shared_ptr<criterion> > &results)
{
    ...
    case L'+':
        ...
        results.push_back(boost::shared_ptr<criterion>(new upperClass(lowerBound)));
        break;
    case L'-':
        processAbsoluteDate(token, lowerBound, upperBound);
        results.push_back(boost::shared_ptr<criterion>(new lowerClass(lowerBound)));
        break;
    case L'=':
        processAbsoluteDate(token, lowerBound, upperBound);
        results.push_back(boost::shared_ptr<criterion>(new upperClass(lowerBound)));
        results.push_back(boost::shared_ptr<criterion>(new lowerClass(upperBound)));
        break;
    case L'!':
        processAbsoluteDate(token, lowerBound, upperBound);
        results.push_back(boost::shared_ptr<criterion>(new lowerClass(upperBound)));

...
namespace scanners
{
	...
	std::wstring getRegexesCommonRoot(std::vector<boost::shared_ptr<regexClass> >& targets)
	{
		//Sanity check:
		...
		for(std::vector<boost::shared_ptr<regexClass> >::const_iterator it = targets.begin(); it != targets.end(); it++)
		{
